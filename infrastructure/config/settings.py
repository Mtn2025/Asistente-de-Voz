...
class Settings(BaseSettings):
    ...
    def validate_db_credentials(cls, v: str, info) -> str:
        ...
        if not v or not v.strip():
             # Fallback to dev defaults if strictly local could be allowed, but for now enforce
             # Actually, let's allow empty if we have defaults above.
             # But the legacy code raised ValueError.
             # We'll keep semantic but be careful with dev env.
    ...
    def validate_admin_key(cls, v: str) -> str:
        ...
        if not v or not v.strip():
             ...
             if os.getenv("APP_ENV") == "test" or APP_ENV == "development":
                 return "test_key"
